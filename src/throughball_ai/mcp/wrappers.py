import asyncio
import time
from typing import Any, Callable, Optional

from throughball_ai.mcp.context import RequestContext


def _is_retryable(exc: BaseException) -> bool:
    return bool(getattr(exc, "retryable", False))


def _degraded_telemetry(
    degraded_reason: str,
    cache_hit: bool = False,
    retry_count: int = 0,
    latency_ms: int = 0,
) -> dict:
    return {
        "degraded": True,
        "degraded_reason": degraded_reason,
        "cache_hit": cache_hit,
        "retry_count": retry_count,
        "latency_ms": latency_ms,
        "external_api_called": False,
    }


def _ok_telemetry(
    cache_hit: bool = False,
    retry_count: int = 0,
    latency_ms: int = 0,
) -> dict:
    return {
        "degraded": False,
        "degraded_reason": None,
        "cache_hit": cache_hit,
        "retry_count": retry_count,
        "latency_ms": latency_ms,
        "external_api_called": False,
    }


async def execute_with_middleware(
    *,
    tool_name: str,
    handler: Callable,
    inputs: dict,
    context: RequestContext,
    timeout_ms: int,
    max_retry_count: int,
    cacheable: bool,
    max_calls: int,
) -> dict:
    # 1. Budget check
    if context.tool_call_count >= max_calls:
        return {
            "ok": True,
            "tool": tool_name,
            "telemetry": _degraded_telemetry("TOOL_BUDGET_EXCEEDED"),
        }

    # 2. Cache check
    if cacheable:
        cached = context.get_cached(tool_name, inputs)
        if cached is not None:
            result = dict(cached)
            result["telemetry"] = dict(cached.get("telemetry", {}))
            result["telemetry"]["cache_hit"] = True
            return result

    context.tool_call_count += 1

    # 3. Retry + timeout loop
    retry_count = 0
    last_exc: Optional[BaseException] = None
    start = time.monotonic()

    while True:
        try:
            result = await asyncio.wait_for(
                handler(**inputs), timeout=timeout_ms / 1000
            )
            elapsed = int((time.monotonic() - start) * 1000)
            result = dict(result)
            result["telemetry"] = _ok_telemetry(
                cache_hit=False, retry_count=retry_count, latency_ms=elapsed
            )
            if cacheable:
                context.set_cached(tool_name, inputs, result)
            return result

        except asyncio.TimeoutError:
            elapsed = int((time.monotonic() - start) * 1000)
            return {
                "ok": True,
                "tool": tool_name,
                "telemetry": _degraded_telemetry(
                    "TIMEOUT", retry_count=retry_count, latency_ms=elapsed
                ),
            }

        except Exception as exc:
            last_exc = exc
            if _is_retryable(exc) and retry_count < max_retry_count:
                retry_count += 1
                await asyncio.sleep(0.2)
                continue
            break

    elapsed = int((time.monotonic() - start) * 1000)
    return {
        "ok": False,
        "tool": tool_name,
        "error": {"message": str(last_exc)},
        "telemetry": {
            "degraded": False,
            "degraded_reason": None,
            "cache_hit": False,
            "retry_count": retry_count,
            "latency_ms": elapsed,
            "external_api_called": False,
        },
    }
